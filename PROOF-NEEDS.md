...
seam; **ASSUMPTION** = stated external-correctness or hardness axiom
(intentionally unprovable in Idris2 — either a cryptographic hardness
postulate or a library-correctness postulate whose discharge target is a
Rust-side property test, not an Idris2 lemma); **OWED** = stated as an
erased obligation the Rust does not yet establish.
...
| I3 | `placePiece` preserves board well-formedness (in-bounds + no overlap) | **DISCHARGED** — `Invariants.placePreservesWF` machine-checked via `allInBoundsGo` structural-induction lemmas + `inBoundsCong`/`allInBoundsGoCong` board-width irrelevance + cons-distribution for `all`; landed in PR #60 |
...
| I7 | Level-pack save/load round-trip identity | **ASSUMPTION** — `Invariants.serdeRoundTripCorrect` is an explicit serde-correctness postulate (analogous to I6's hardness axiom but for library correctness, not cryptography); `levelRoundTrip` is retained as a derived alias for downstream callers. The Rust-side proof target is a serde property test, NOT an Idris2 lemma; landed in PR #62 |
...
| Pack round-trip (I7, Rust-side) | serde `Serialize`/`Deserialize` for `Level` round-trips under proptest | Community-pack corruption across disk; converts I7 from `ASSUMPTION` to (proptest-grade) discharged | I7 (Rust proptest, optional follow-up) |
...
I3 and I5 discharged (`placePreservesWF`, `decNonDecreasing` +
`builtinPackMonotone`), I6 stated as an explicit cryptographic-hardness
assumption, I7 stated as an explicit serde-correctness assumption (with
a Rust-side proptest tracked as the discharge path), and I1, I2, I4
